...
function Footer() {
  return (
    <footer className="bg-white border-t border-gray-200 w-full mt-16">
      <div className="max-w-[1440px] mx-auto px-4 pt-10 pb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8">
          ...
          {NAV_COLUMNS.map(col => (
            <div key={col.title}>
              <div className="text-sm font-semibold text-gray-800 mb-3">{col.title}</div>
              <ul className="space-y-2.5">
                {col.links.map(l => (
                  <li key={l.to}>
                    <Link to={l.to} className="text-sm text-gray-600 hover:text-brand-600 transition-colors">
                      {l.label}
                    </Link>
                  </li>
                ))}
          ...
          <div>
            <div className="text-sm font-semibold text-gray-800 mb-3">안내</div>
            <ul className="space-y-2.5">
              <li>
        ...
        <div className="mt-10 pt-6 border-t border-gray-100 space-y-2">
          <p className="text-xs leading-relaxed text-gray-500">
            본 서비스는 <b className="font-medium text-gray-700">정보 제공 목적</b>이며 투자 권유가 아닙니다.
            <br/>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
            <span className="text-xs text-gray-500">© 2026 UPquant · 비상업 개인 프로젝트</span>
          </div>
        </div>
      </div>
    </footer>
