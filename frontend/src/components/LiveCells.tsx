...
const changeColor = (ch: string) => (ch === 'RISE' ? 'text-red-500' : ch === 'FALL' ? 'text-blue-500' : 'text-gray-600 dark:text-gray-300')

export function LivePrice({ ticker, className = '' }: { ticker: Ticker; className?: string }) {
...
export function LiveChangeRate({ ticker, className = '' }: { ticker: Ticker; className?: string }) {
  const live = useLivePrice(ticker.market)
