...
const GUIDE = [
  {
    ...
    features: [
      ...
      { tone: 'nav',    name: '투자 시그널 패널', desc: '모멘텀 롱 후보·공적분 페어 진입(|z|>2)·시장 국면 전환·52주 돌파/급등을 한곳에 모읍니다. 클릭 시 해당 분석/종목으로 이동합니다. 인샘플·생존편향·거래비용 한계가 있는 후보이며 매매 보장이 아닙니다.' },
      ...
      { tone: 'action', name: '디지털 자산 표 (기간별 / 시가총액)', desc: '[기간별 상승률]은 1주~1년 수익률, [시가총액]은 외부(CoinGecko) 시총 기준입니다. 기간 헤더를 클릭하면 그 기간으로 정렬되고, [CSV 내보내기]로 표를 파일로 받을 수 있습니다. 행 클릭 시 상세로 이동합니다.' },
  ...
  },
  {
    ...
    features: [
      ...
      { tone: 'action', name: 'CSV 내보내기', desc: '걸러진 결과를 CSV 파일로 내보냅니다. 어떤 조건으로 걸렀는지에 따라 관련 지표(BTC 베타·거래량 급증·변동성 z) 컬럼이 함께 표시됩니다.' },
  ...
  },
  {
    ...
    features: [
      ...
      { tone: 'nav',    name: '최근 본 코인', desc: '넓은 화면에서는 왼쪽에 최근 조회한 코인(최대 6개)이 플로팅으로 떠, 클릭하면 바로 이동합니다.' },
    ],
  },
  {
    route: '/coins/KRW-BTC',
    ...
    features: [
      ...
      { tone: 'nav',    name: '시그널 배지', desc: '이 종목에 해당하는 투자 시그널(모멘텀 롱·페어 진입·국면 전환·돌파/급등)이 있으면 배지로 표시하고, 클릭 시 해당 분석으로 이동합니다.' },
  ...
  {
    route: '/strategy/backtest/ma',
    title: '백테스트 (전략)',
    summary: '실제 매매 전략을 과거 데이터로 시뮬레이션합니다(전략 실행).',
    features: [
      { tone: 'nav', name: '전략 선택 (헤더 드롭다운)', desc: '헤더 [백테스트▾]에서 MA 크로스·RSI 역추세·추세추종(TSMOM)·포트폴리오 보유를 고릅니다. 각각 별도 URL(/strategy/backtest/…)이라 딥링크·공유가 됩니다.' },
  ...
  },
  {
    ...
    features: [
      { tone: 'nav', name: '기법 선택 (헤더 드롭다운)', desc: '헤더 [검증▾]에서 전략 비교·워크포워드·몬테카를로를 고릅니다(각각 별도 URL).' },
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#141b29]">
      ...
      <div className="bg-white dark:bg-[#1a2234] border border-gray-200 dark:border-[#2c3850] rounded-md p-5">
        ...
        <ul className="text-sm text-gray-500 dark:text-gray-400 space-y-1.5 list-disc pl-5 leading-relaxed">
          ...
          <li>접속에는 <span className="font-medium text-gray-700 dark:text-gray-200">로그인</span>이 필요합니다(과제용 단일 계정 <span className="font-mono">test / test</span>). 로그아웃은 우측 상단 <span className="font-mono">⋯</span> 메뉴에 있습니다.</li>
          <li>헤더 우측 도구 — <span className="font-medium text-gray-700 dark:text-gray-200">🔔 가격 알림</span>(목표가 도달 시 토스트)·<span className="font-medium text-gray-700 dark:text-gray-200">🌙 다크 모드</span>·<span className="font-medium text-gray-700 dark:text-gray-200">✦ AI 전략 리포트</span>(Gemini)·<span className="font-mono">⋯</span> 더보기(분석 가이드·도움말·시스템 모니터링·실시간 상태·로그아웃).</li>
        </ul>
      </div>
