    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-[#1a2234] border border-gray-200 dark:border-[#2c3850] rounded-md p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {PRESETS.map(p => (
            <button
              ...
              className="px-3 py-1 text-xs border border-gray-200 dark:border-[#2c3850] rounded-full text-gray-500 dark:text-gray-400 cursor-pointer hover:border-brand-400 hover:text-brand-500 transition-colors"
            >
              {p.label}
            </button>
          ))}
        ...
        <div className="space-y-2 mb-4">
          {conditions.length === 0 && (
            <div className="text-sm text-gray-400 dark:text-gray-500 py-2">아래 버튼으로 조건을 추가하거나 프리셋을 선택하세요</div>
          )}
          {conditions.map(cond => (
            <div key={cond.id} className="flex items-center gap-2">
              <select
                ...
                className="border border-gray-200 dark:border-[#2c3850] rounded px-2.5 py-1.5 text-sm cursor-pointer focus:outline-none focus:border-brand-400"
              >
              ...
              <input
                ...
                placeholder="값"
                className="w-24 border border-gray-200 dark:border-[#2c3850] rounded px-2.5 py-1.5 text-sm focus:outline-none focus:border-brand-400"
              />
              <span className="text-xs text-gray-400 dark:text-gray-500 w-4">{FIELDS.find(f => f.key === cond.field)?.unit}</span>
              <button
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-4 py-1.5 border border-gray-200 dark:border-[#2c3850] text-sm text-gray-600 dark:text-gray-300 rounded cursor-pointer hover:bg-gray-50 transition-colors"
          >
            + 조건 추가
          </button>
          <button
          ...
          {(conditions.length > 0 || results !== null) && (
            <button
              onClick={handleReset}
              className="px-4 py-1.5 text-sm text-gray-400 dark:text-gray-500 cursor-pointer hover:text-gray-600 transition-colors"
            >
              초기화
            </button>
          )}
                ...
                {results.map(coin => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="font-medium text-gray-800 dark:text-gray-100">{coin.market.replace('KRW-', '')}</div>
                      <div className="text-xs text-gray-400 dark:text-gray-500">{coin.korean_name}</div>
                    </td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-200 font-medium">
                      {coin.trade_price.toLocaleString()}
                    </td>
                    <td className={`px-4 py-3 text-right font-medium ${
                      coin.change_rate > 0 ? 'text-red-500' : coin.change_rate < 0 ? 'text-blue-500' : 'text-gray-400 dark:text-gray-500'
                    }`}>
                    ...
                    <td className={`px-4 py-3 text-right font-medium ${
                      coin.return_1m > 0 ? 'text-red-500' : coin.return_1m < 0 ? 'text-blue-500' : 'text-gray-400 dark:text-gray-500'
                    }`}>
                      {(coin.return_1m > 0 ? '+' : '') + coin.return_1m.toFixed(2)}%
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-16 h-1.5 bg-gray-100 dark:bg-[#222c3e] rounded-full overflow-hidden">
                          <div
