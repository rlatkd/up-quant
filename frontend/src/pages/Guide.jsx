...
function ImagePlaceholder({ label }) {
  return (
    <div className="mt-3 flex flex-col items-center justify-center h-32 rounded-md border-2 border-dashed border-gray-300 bg-gray-50 text-gray-400">
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white border border-gray-200 rounded-md p-6">
          <h1 className="text-lg font-bold text-gray-800 mb-1.5">분석 가이드 · 방법론</h1>
          <p className="text-sm text-gray-500 leading-relaxed">
            UPquant이 제공하는 정량 분석이 각각 <span className="font-medium text-gray-700">무엇을 답하는지(개념)</span>와
            사용한 <span className="font-medium text-gray-700">기법·라이브러리</span>를 정리했습니다.
          ...
          <p className="text-xs text-gray-400 mt-2">※ 각 항목의 [열기 →]를 누르면 메인 창이 해당 분석 화면으로 이동합니다. 개념 다이어그램은 추후 추가 예정(현재 placeholder).</p>
        </div>

        {/* 방법론 카드 */}
        {METHODOLOGY.map(m => (
          <div key={m.name} className="bg-white border border-gray-200 rounded-md p-5">
            ...
            <div className="text-sm text-gray-600 leading-relaxed">{m.q}</div>
            <div className="text-xs text-gray-400 mt-1">방법 · {m.how}</div>
