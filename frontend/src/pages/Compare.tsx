...
export default function Compare() {
  ...
  useEffect(() => {
    setSearchParams(selected.length ? { markets: selected.join(',') } : {}, { replace: true })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-[#1a2234] border border-gray-200 dark:border-[#2c3850] rounded-md p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">최대 5개 · {selected.length}/5 선택됨</div>
          </div>
          <div className="flex items-center gap-2">
            <input
              ...
              className="border border-gray-200 dark:border-[#2c3850] rounded px-2.5 py-1.5 text-sm w-48 focus:outline-none focus:border-brand-400"
            />
            {selected.length > 0 && (
              <button
                onClick={() => setSelected([])}
                className="px-3 py-1.5 text-sm text-gray-400 dark:text-gray-500 cursor-pointer hover:text-gray-600 transition-colors"
              >
                초기화
              </button>
            )}
          ...
          <div className="grid grid-cols-4 gap-1 p-2">
            {filtered.map(t => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-medium text-gray-800 dark:text-gray-100">{t.market.replace('KRW-', '')}</span>
                  <span className="text-xs text-gray-400 dark:text-gray-500 truncate w-full">{t.korean_name}</span>
                </button>
              )
            })}
            {filtered.length === 0 && (
              <div className="col-span-4 text-center text-xs text-gray-400 dark:text-gray-500 py-6">검색 결과 없음</div>
            )}
          </div>
        </div>
      </div>
      ...
      {/* 비교 차트 */}
      <div ref={chartRef} className="bg-white dark:bg-[#1a2234] border border-gray-200 dark:border-[#2c3850] rounded-md p-5">
        <div className="flex items-center justify-between mb-0.5">
          <div className="flex items-center gap-2">
            <div className="text-sm font-semibold text-gray-700 dark:text-gray-200">수익률 비교</div>
            {loadingChart && chartData.length > 0 && (
              <div className="w-3.5 h-3.5 border-2 border-gray-200 dark:border-[#2c3850] border-t-brand-500 rounded-full animate-spin" />
            )}
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={share} disabled={selected.length === 0}
              className="px-2.5 py-1 text-xs rounded border border-gray-200 dark:border-[#2c3850] text-gray-500 dark:text-gray-400 hover:bg-gray-50 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
              {copied ? '복사됨!' : '🔗 공유 링크'}
            </button>
            <button onClick={() => downloadChartPng(chartRef.current, 'upquant-compare.png')} disabled={chartData.length === 0}
              className="px-2.5 py-1 text-xs rounded border border-gray-200 dark:border-[#2c3850] text-gray-500 dark:text-gray-400 hover:bg-gray-50 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
              PNG
            </button>
          </div>
        </div>
        <div className="text-xs text-gray-400 dark:text-gray-500 mb-4">90일 기준 초기값 대비 누적 등락률 (%)</div>
        {selected.length === 0 ? (
          <div className="h-64 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500">종목을 선택하세요</div>
            ...
            return (
              <div key={m} className="bg-white dark:bg-[#1a2234] border border-gray-200 dark:border-[#2c3850] rounded-md p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{m.replace('KRW-', '')}</span>
                  <span className="text-xs text-gray-400 dark:text-gray-500">{ticker?.korean_name}</span>
                </div>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between">
                    <span className="text-gray-400 dark:text-gray-500">90일 수익률</span>
                    <span className={`font-semibold ${last >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
                      {last >= 0 ? '+' : ''}{last.toFixed(2)}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400 dark:text-gray-500">최고 수익률</span>
                    <span className={`font-medium ${maxVal >= 0 ? 'text-red-500' : 'text-blue-500'}`}>{maxVal >= 0 ? '+' : ''}{maxVal.toFixed(2)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400 dark:text-gray-500">최저 수익률</span>
