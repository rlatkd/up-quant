...
function AlertWatch({ alert, onTrigger }: any): any {
...
function ToastContainer({ toasts, dismiss }: any) {
...
export function PriceAlertProvider({ children }: { children: ReactNode }) {
  ...
  const addAlert = useCallback((a: any) => {
    setAlerts((prev: any) => [...prev, { ...a, id: Date.now() + Math.random() }])
  }, [])
  const removeAlert = useCallback((id: any) => {
    setAlerts((prev: any) => prev.filter((x: any) => x.id !== id))
  }, [])
  const dismiss = useCallback((id: any) => setToasts(prev => prev.filter(t => t.id !== id)), [])

  const handleTrigger = useCallback((alert: any, price: any) => {
    ...
    setAlerts((prev: any) => prev.filter((x: any) => x.id !== alert.id))  // 일회성 — 충족 후 제거
  ...
  return (
    <PriceAlertContext.Provider value={{ alerts, addAlert, removeAlert }}>
      {children}
      {alerts.map((a: any) => <AlertWatch key={a.id} alert={a} onTrigger={handleTrigger} />)}
