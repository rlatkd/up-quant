...
**다음 작업 (우선순위 순) — 이번 세션 미완/이월**
1. [ ] ⭐ **포트폴리오 동선 통합 (사용자 요청)** — 포트폴리오 기능이 3곳 분절(퀀트 랩 Markowitz 최적비중·백테스트 포트폴리오 수동비중·비교분석 수익률겹침). **퀀트 랩 최적비중(★최대샤프/최소분산)을 백테스트·비교로 넘기는 동선** 구축("최적화→검증→비교"). (엔지니어링노트 §28)
2. [ ] **실제 화면 검증(브라우저 육안)** — Phase 12~20 누적 + 신규(퀀트 랩 8탭·탐색 통합·코인상세 지표·포트폴리오 백테스트) 미검증. `npm run dev`
3. [ ] **WebSocket 실시간 시세** — `wss://api.upbit.com/websocket/v1` → FastAPI WS 중계 → 프론트 Context (+가격 깜빡임 펄스)
4. [ ] **에러/로딩 상태 UI 개선**
5. [ ] **카테고리 잔여** — 산점도 섹터별 색 · 누적 변동성 드래그 · 분류 스냅샷 자동 갱신
