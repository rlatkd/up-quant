...
헤더 탭을 **의도별 4그룹**으로 묶은 SPA입니다 — *요약*(대시보드) │ *탐색*(마켓·섹터·스크리너) │ *정량분석*(시장 구조·시장 국면·팩터 분석·리스크) │ *전략 도구*("서비스 더보기" 드롭다운: 포트폴리오 최적화·백테스트·비교 분석). 로고(`/`)는 **코인 목록**(master-detail)이며, 도움말·가이드는 별도 창입니다.
...
| `/market` | **마켓 현황** | 미니 차트 카드 · 52주 신고/신저 배지(상위 30) · 상승/하락/거래대금 표 · 거래대금 트리맵 · 리스크-수익 산점도 |
...
| `/tools/portfolio` | **포트폴리오 최적화** | Markowitz 효율적 경계선(구름 + 곡선 + ★최대샤프/◆최소분산), → 백테스트로 비중 전달 |
| `/tools/backtest` | **백테스트** | MA크로스·RSI·**전략 비교**·**워크포워드**·포트폴리오 보유 / 거래비용·buy&hold/BTC 벤치마크·알파·Sharpe/Sortino/Calmar |
| `/tools/compare` | **비교 분석** | 최대 5종목 누적 등락률 겹쳐 비교 |
