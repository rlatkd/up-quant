...
업비트 KRW 마켓의 시세·호가·체결·캔들 데이터를 한 화면에서 탐색하고, 카테고리별 수익률과 리스크-수익 분포를 분석하는 대시보드입니다. 현재 백엔드는 결정론적 **더미 데이터**로 동작하며, 외부 API 클라이언트 한 곳만 교체하면 실제 업비트 Open API로 전환되도록 설계되어 있습니다.
...
| 경로 | 페이지 | 설명 | 참고 스타일 |
|------|--------|------|------------|
| `/` | **Dashboard** | KPI 카드 · 카테고리 누적수익률 라인차트 · 월별 수익률 히트맵 · 리스크-수익 산점도 | Finviz |
| `/market` | **Market** | 미니 차트 카드 · 상승률 테이블 · 거래대금 트리맵 | — |
| `/coins` | **CoinList** | 시장 요약 카드 · 검색 · 코인 테이블(스파크라인) | CoinGecko |
| `/coins/:market` | **CoinDetail** | 캔들차트 · 실시간 호가창 · 체결 내역 | Upbit |
...
**완료**
- [x] 백엔드 3개 라우터 (markets / candles / analysis) + 응답 스키마
- [x] 15개 KRW 코인 더미 데이터 · 결정론적 캔들/스파크라인
- [x] 카테고리 월별·누적 수익률, 변동성·1개월 수익률 계산
- [x] 프론트 4개 페이지 + 데이터 페칭 훅
