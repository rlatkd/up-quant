...
**Phase 19 — 퀀트/ML 분석 묶음 + 퀀트 랩 (2026-05-31) — 완료**
- [x] **라이브러리 도입** — numpy·scipy·scikit-learn·statsmodels·arch·hmmlearn·networkx ("있는 모델은 라이브러리, 없는 건 numpy"). 정량 분석은 **예측보다 구조·리스크** 중심.
- [x] **정량 분석 8종** (`/api/quant/*`, `quant_service.py` 공용 `returns_matrix` 헬퍼) — Markowitz 효율적 경계선(P1-2)·상관 네트워크 MST(P3-1)·PCA 시장요인·K-means+계층 덴드로그램(P3-2)·GARCH 변동성예측+VaR·횡단면 모멘텀 팩터 백테스트·공적분 페어트레이딩·HMM 시장국면
- [x] **퀀트 랩 페이지** (`/quant`, 헤더 별도 탭) — 8 서브탭, d3-force 네트워크·SVG 덴드로그램·recharts. 부팅 프리페치 9종 워밍(기존 일봉캐시 재사용 → 팬아웃0)
- (브라우저는 네트워크 탭 1건만 수정 검증, 나머지 7탭 런타임 미검증)

**Phase 20 — 탐색 통합 + 코인상세 강화 + 포트폴리오 백테스트 (2026-05-31) — 완료**
- [x] **P2-1 탐색 통합** — Market·Sectors·Screener를 `/explore` 단일 페이지 URL기반 서브탭으로(`Explore.jsx`), 헤더 `대시보드·탐색·코인목록`으로 단순화
- [x] **P2-2 코인 상세 강화** — 주요 지표 카드(30일 변동성·1개월 수익률·시장 점유율·GARCH 연변동성·1일 VaR) + 52주 위치 바
- [x] **P2-3 포트폴리오 백테스트** — `/api/backtest/portfolio`(가중 보유 + 리밸런스 + 동일가중 벤치마크), 프론트 `PortfolioBacktest`(카트 종목·비중·기여도)

**Phase 21~22 — IA 정리 + 리스크 탭 + 거래비용 + 대시보드 관제탑 (2026-06-01~03) — 완료**
- [x] 퀀트 랩(`/quant`) 해체 → 정량분석을 `/structure`(시장 구조)·`/factor`(팩터)·`/risk`(리스크) 헤더 탭으로 분산(`Analysis.jsx`)
- [x] 코인 목록=메인(`/`), 대시보드=`/dashboard`, 가이드 새 창 `/guide`
- [x] 상폐 종목 404→500 버그 수정(valid_markets 교집합 + 404→[] 안전망)
- [x] 리스크 탭(변동성 분포·정규근사 1일 95% VaR 랭킹, coinStats 재사용 호출0)
- [x] 거래비용 `fee_bps`(기본 5bps) 백테스트·모멘텀 반영, 단일전략 buy&hold 벤치마크+알파
- [x] 대시보드 관제탑 재구성(KPI4 + 시장 종합 추세 focal + 오늘의 시그널 + 보조 4카드 + 시세표)
- [x] 전략도구 "More" 드롭다운 + 독립 라우트(`/tools/portfolio·backtest·compare`)

**Phase 23 — 헤더 한글화 + 차트 버그수정 + 섹터 누적 일봉화 + 공통 푸터 (2026-06-03) — 완료**
- [x] 헤더 라벨 한글 전문음차(대시보드·마켓·섹터·스크리너·시장 구조·팩터 분석·리스크·서비스 더보기)
- [x] **RSI 클릭 시 흰 화면 크래시 수정** — lightweight-charts v5에서 `getSeries()`가 `IPaneApi` 소속으로 이동(chart엔 없음) → ref 추적으로 교체 + StrictMode stale ref 초기화. 차트 attributionLogo 끄기(layout 안), 지표 토글 시 줌 리셋 수정
- [x] **섹터 누적수익률 월봉→일봉** — `get_category_daily_cumulative`(섹터 일봉 동일가중 지수, 최근 ~200일, 공용 캐시 재사용 호출0), recharts 표준 Tooltip+activeDot, 기간 선택기 제거, Y축 상단 50% 보장
- [x] 대시보드 섹터 성과 막대 카테고리별 색상, 코인목록 사이드바 컬럼 폭·정렬 화살표 정리
- [x] 팩터/백테스트 신뢰성 경고 제거, 로고 파란점 제거(PIL), 배경색 `#e6eaf2`
- [x] **공통 푸터 신설**(`Footer.jsx`) — full-bleed 흰 배경, logo CSS mask 네이비, 네비 3컬럼+안내+면책

**Phase 24 — 효율적 경계선 곡선 + 시장구조 2분리 + 신뢰성 경고 정비 + 성능/죽은코드 정리 (2026-06-06) — 완료**
- [x] **효율적 경계선 곡선** — 목표수익률 60등분 최소분산 최적화로 frontier 곡선 추가(`PortfolioResult.frontier`) + Dirichlet α 0.3으로 구름 확산. "원 뭉침" 진단: 메이저 상관 높음(정상) + 균등샘플 중심집중(개선)의 복합
- [x] **시장 구조 → 2페이지 분리** — `/structure`(상관 네트워크+클러스터링, 종목 간 관계) · `/regime`(PCA 요인+HMM 국면, 시장 전체 상태). 헤더 탭·라우트·크로스링크 정리
- [x] **신뢰성 경고** — 복원(3)+추가(3) 했다가 사용자 요청으로 **전부 제거**(amber 경고 박스 6곳 + 모멘텀 툴팁 ⚠️). Phase 23 상태로 환원
- [x] **성능/정리** — 스파크라인 261→상위30종만 fetch(`_SPARK_LIMIT`), 죽은 월/분기/년 누적 코드 체인 제거, 상관계산 min_len(40일) 필터, 죽은 `PageHeader.jsx` 삭제
- [x] **코인리스트 줄아웃 높이 버그** — wrapper + `absolute inset-0`로 우측 리스트를 좌측 상세 높이에 고정(사용자 확인 완료)
- [x] **에러 바운더리 신설** — `ErrorBoundary.jsx`로 한 페이지 예외가 전체 화면을 죽이지 않게(Phase 23 흰화면 계기), 라우트 변경 시 자동 복구
- [x] **코인 상세 보강(호출 0)** — 거래대금 순위 Metric + 호가 매수/매도 압력 바
- [x] **분석 카트 전면 제거** — Context·CartButton·헤더 인디케이터·5페이지 + 버튼 제거(자체 선택 UI로 대체 가능·즐겨찾기와 중복)
- [x] **전수 검토 + 발표 주제 정합성 확인** — 9기법·5단계 흐름 전부 구현됨 확인

**Phase 25 — 실시간 WebSocket 시세 중계 + 백테스트 강화 + 통합 로딩 컴포넌트 (2026-06-07) — 완료**
- [x] **실시간 시세 중계 허브** — 업비트 ticker WS **1개**를 모든 클라이언트에 fan-out(`main.py:TickerHub`, `/ws/tickers`), 신규 연결 시 스냅샷 즉시 푸시·구독자 0이면 중단·끊기면 자동 재연결
- [x] **프론트 실시간 인프라** — 외부 store(`realtimeStore.js`)+`useSyncExternalStore` 종목별 selector(한 종목만 리렌더), 300ms 배치(`RealtimeProvider`), `LivePrice`/`LiveChangeRate` 셀(REST 폴백 + 변동 펄스), 헤더 연결 인디케이터. 대시보드·코인목록·코인상세 적용
- [x] **코인 상세 호가·체결 실시간** — 종목별 WS(`/ws/market/{market}`·`useMarketStream.js`), REST 폴백 위에 덧씌움
- [x] **백테스트 강화** — 전략 비교(MA·RSI 겹쳐보기 `/compare`), 워크포워드(in-sample 그리드서치→out-of-sample 검증 `/walk-forward`), BTC 매수보유 벤치마크(`benchmark_btc`)
- [x] **통합 로딩 컴포넌트** — `PageLoading.jsx`를 8개 페이지 공용으로

**다음 작업 (우선순위 순)**
1. [ ] **브라우저 육안 검증** — Phase 24~25 변경분(효율적 경계선 곡선·시장구조 분리·실시간 펄스/연결 인디케이터·전략 비교·워크포워드·코인리스트 줄아웃) 미검증
2. [ ] **실시간 확장** — 코인목록/마켓 순위까지 실시간 정렬·갱신, WS 재연결 백오프 튜닝, 체결 스트림 시각화 보강
3. [ ] **표시 종수 일관성** — 요약(10) vs 분석(VaR 30·산점도 120) 구분 명문화(현재 문제 아님)
