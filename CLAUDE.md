...
- 라우트(**Phase 21~30 기준**): 로고(`/`)=**코인 목록**(master-detail, `/coins`·`/coins/:market`도 동일). `/dashboard`(시황 대시보드, 별도 경로). 탐색계열 `/market`·`/sectors`·`/screener`(+`/explore`)는 `Explore` 단일 페이지의 URL 서브탭. 정량분석 `/structure`(**시장 구조: 상관네트워크·군집** — 종목 간 관계/미시)·`/regime`(**시장 국면: PCA 요인·HMM 국면** — 시장 전체 상태/거시)·`/factor`(팩터: 모멘텀·페어)·`/risk`(리스크: 변동성분포·VaR)는 `Analysis` 한 컴포넌트(Phase 30부터 **경로별 Body 컴포넌트**가 그 그룹 훅을 모아 게이트 — `PAGE_META` 제거). 전략도구 `/tools/portfolio`(최적화)·`/tools/backtest`(전략 실행: MA·RSI·추세추종·포트폴리오 보유)·`/tools/validation`(검증·시뮬: 전략비교·워크포워드·몬테카를로, **Phase 30 백테스트에서 분리**)·`/tools/compare`(종목 비교, 독립 페이지). 옛 `/quant`·`/analysis/*`·`/compare`·`/backtest`는 리다이렉트. `/help`·`/guide`만 `window.open` 별도 창(Layout 밖). **헤더(`sticky top-0 z-50`, 한글 라벨, Phase 30 자산운용 톤 재명명)**: 최상위 4그룹 — 로고(→코인목록) · **시황**(단일,/dashboard) · **마켓▾**(시장 현황·섹터·스크리너) · **리서치▾**(시장 구조·시장 국면·팩터·리스크·**종목 비교**) · **전략▾**(최적화·백테스트·검증·시뮬레이션). 마켓/리서치/전략은 호버 드롭다운(`NavGroup`). 그룹명은 증권사·운용사 리서치 톤(리서치=분석조직·리포트, 포트폴리오/전략=구성·검증). **종목 비교는 "고른 종목 분석"이라 리서치로, 스크리너는 "전체 시장 발굴"이라 마켓에** 배치. 우측은 **아이콘화 + 더보기(⋯)** — 🔔가격알림 · 🌙다크모드 · ✦AI리포트 상시, **⋯ 메뉴**(가이드·도움말·시스템·실시간상태). 메뉴 글씨 `15.5px`·항목 패딩 `px-5`. ⚠️ **원칙: 페이지 내 탭/토글로 콘텐츠를 숨기지 않는다**(드롭다운은 내비 한정 / 백테스트·검증의 전략선택은 "무엇을 계산할지" 기능적 선택이라 예외). (헤더 진화: 6탭→4+드롭(16)→탐색통합(20)→평탄7+그룹(21)→More드롭·영어화(22)→한글화(23)→시장구조2분리(24)→4그룹 드롭다운+우측⋯(29)→**자산운용 톤 재명명·종목비교 리서치 이동·전략 분리(30)**. 엔지니어링노트 §22·§30·§44·§45.)
...
- 사용자와 **한국어**로 소통.
- **커밋은 맡기면 직접 수행해도 된다**(2026-06-09 변경). 단 ⑴메시지는 **한 줄**(한글 conventional: `feat:`/`fix:`/`refactor:`/`chore:`/`docs:`/`test:`, 여러 항목은 `·`로 나열), ⑵**Claude/Co-Authored-By를 절대 명시하지 말 것**(사용자가 직접 작성한 것처럼), ⑶**도메인별로 파일을 묶어** 여러 커밋으로 나눌 것. `push`는 사용자가 따로 지시할 때만.
