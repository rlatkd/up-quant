...
**다음 작업 (우선순위 순)**
1. ~~배포(AWS)~~ **완료(Phase 36)** — S3+CloudFront(FE)·EC2(BE)·Route53·ACM·도메인 `skku.site`, GitHub Actions CI/CD 4워크플로 라이브. 운영 `www.skku.site`↔`api.skku.site`. (상세·운영주의는 작업 이력 Phase 36)
...
### Phase 36 — AWS 실배포 + CI/CD 파이프라인 + 배포 후속 UI 마무리 (2026-06-11)
실제 배포 완료 — **S3+CloudFront(정적 FE)·EC2(FastAPI)·Route53·ACM·가비아 도메인 `skku.site`**. 운영 주소 **`www.skku.site`(FE) ↔ `api.skku.site`(EC2)**.
- **CI/CD(GitHub Actions, 4워크플로 분리)**: `ci-frontend`·`ci-backend`(push 시 lint/typecheck/vitest · compileall/pytest) → 성공 시 **`workflow_run` 체이닝**으로 `cd-frontend`(빌드→S3 sync→CloudFront 무효화)·`cd-backend`(EC2 SSH→git pull→Secrets로 `.env` 주입→pip→restart→**헬스 ready 폴링**). path 필터는 CI 쪽에. 비밀은 **GitHub Repository Secrets 단일소스**(FE는 빌드타임 `vars.VITE_*`). 옛 ci.yml+합쳐진 deploy.yml은 4분리로 대체. 배포 스크립트·systemd·nginx는 **`.github/deploy/`** 로 이동.
- **EC2(Ubuntu) 부트스트랩**(`.github/deploy/setup-ec2.sh`): t3.small·30GiB gp3·스왑2G, **uvicorn 단일프로세스 systemd**(⚠️인메모리 캐시·WS 허브가 프로세스 상태라 멀티워커 금지), nginx 리버스프록시(REST+WS 같은 호스트 `api.skku.site`)·certbot TLS. `CORS_ORIGINS=https://www.skku.site`·`COOKIE_SECURE=1`.
- **CloudFront 403 해결**: 원본이 **S3 website 엔드포인트**(공개용)인데 버킷은 OAC(비공개)라 AccessDenied → **REST 엔드포인트+OAC**로 교체. apex(`skku.site`)→www는 **CloudFront Function 301 리다이렉트**(CORS는 www만 유지). 403/404→`/index.html`(SPA 라우팅).
- **배포 후 UI**: AI 리포트 면책 리포트별→**하단 하나**로 통합, 시장 동향 랭킹 스택 **5→6행**(간격만 좁힘·영역 위아래 고정), **로그인 후 항상 메인('/')**.
- 검증: 프론트 `tsc·eslint·vitest 9`·백엔드 `pytest 43` 그린. **www.skku.site 실서비스 동작 확인**. ⚠️운영: EC2 repo는 main 추적, `.env`는 CD가 Secrets로 매번 덮어쓰니 **GitHub Secret과 값 일치 필수**(특히 `AUTH_SECRET` — 바뀌면 전원 로그아웃). Windows dev: `fastapi dev`(--reload)는 저장마다 프리페치 재시작 → `SKIP_PREFETCH=1`.
