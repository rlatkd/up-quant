...
- 라우트(**Phase 21~24 기준**): 로고(`/`)=**코인 목록**(master-detail, `/coins`·`/coins/:market`도 동일). `/dashboard`(대시보드, 별도 경로). 탐색계열 `/market`·`/sectors`·`/screener`(+`/explore`)는 `Explore` 단일 페이지의 URL 서브탭. 정량분석 `/structure`(**시장 구조: 상관네트워크·군집** — 종목 간 관계/미시)·`/regime`(**시장 국면: PCA 요인·HMM 국면** — 시장 전체 상태/거시, Phase 24 분리)·`/factor`(팩터: 모멘텀·페어)·`/risk`(리스크: 변동성분포·VaR)는 `Analysis` 단일 페이지(`PAGE_META`로 경로→그룹 매핑). 전략도구 `/tools/portfolio`·`/tools/backtest`·`/tools/compare`(독립 페이지). 옛 `/quant`·`/analysis/*`·`/compare`·`/backtest`는 리다이렉트. `/help`·`/guide`만 `window.open` 별도 창(Layout 밖). **헤더(`sticky top-0 z-50`, 한글 라벨, Phase 29 그룹 드롭다운 재편)**: 평탄 8탭이 과해 **최상위 4개로 축소** — 로고(→코인목록) · **대시보드**(단일) · **둘러보기▾**(마켓·섹터·스크리너) · **분석▾**(시장 구조·시장 국면·팩터 분석·리스크) · **전략▾**(포트폴리오 최적화·백테스트·비교 분석). 둘러보기/분석/전략은 호버 드롭다운(`NavGroup`), 하위는 기존 라우트 그대로. 우측은 **아이콘화 + 더보기(⋯)** — 🔔가격알림 · 🌙다크모드 · ✦AI리포트 상시, **⋯ 메뉴**(가이드·도움말·시스템 모니터링·실시간 연결상태). ⚠️ **원칙: 페이지 내 탭/토글로 콘텐츠를 숨기지 않는다**(드롭다운은 내비 한정 / 백테스트 전략선택은 "무엇을 계산할지" 기능적 선택이라 예외). (헤더 진화: 6탭→4+드롭(16)→탐색통합(20)→평탄7+그룹(21)→More드롭·영어화(22)→한글화(23)→시장구조2분리(24)→**4그룹 드롭다운+우측⋯(29)**. 엔지니어링노트 §22·§30·§44.)
...
**다음 작업 (우선순위 순)**
1. **브라우저 육안 검증** — Phase 24~29 변경분(다크모드·가격알림·AI리포트·몬테카를로·추세추종·A-D·시스템·효율적경계선·실시간펄스·에러재시도 + **Phase 29 코인동향 대시보드(지수카드 당일/전일·환율·뉴스·체결강도·자산지수 탭)·헤더 드롭다운·마켓 요약스트립**) 미검증. **신규 백엔드 라우트는 서버 재기동 필요**(트렌드는 60분봉 팬아웃으로 부팅↑ → dev `SKIP_PREFETCH=1`).
